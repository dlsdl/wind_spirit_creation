...
const hexdigit = ["00", "01", "02", "03", "04", "05", "06", "07", "08", "09", "0a", "0b", "0c", "0d", "0e", "0f",
...
function createPlayer() {
    return {
        ...
        hdrn30w: new Decimal(0),
        hdrn40w: new Decimal(0),
        hdrn60w: new Decimal(0),
        wxzf: new Decimal(0),
        hydf: new Decimal(0),
        xkwz: new Decimal(0),
        zzlt: new Decimal(0),
        ...
        Rlitengp: false,
        Blitengp: false,
        glucam: new Decimal(0),
        frucam: new Decimal(0),
        staram: new Decimal(0),
        sucram: new Decimal(0),
        ...
        slwb01: new Decimal(0),
        slwb02: new Decimal(0),
        slwb03: new Decimal(0),
        slwb04: new Decimal(0),
        slwb05: new Decimal(0),
        slwb06: new Decimal(0),
        slwb07: new Decimal(0),
        slwb08: new Decimal(0),
        magstolv: 0,
        magstorm: false,
        ...
        hasUnlockedBases: false,
        baseA_unlocked: [false, false, false, false, false],
        baseG_unlocked: [false, false, false, false, false],
...
function createV() {
    return {
        ...
        upgcor05: new Decimal(4096),
        upgcor06: new Decimal(8192),
        upgcor07: new Decimal(16384),
        upgcor08: new Decimal(32768),
        upgcsl01: new Decimal(512),
        upgcsl02: new Decimal(1024),
        upgcsl03: new Decimal(2048),
        upgcsl04: new Decimal(4096),
        upgcsl05: new Decimal(8192),
        upgcsl06: new Decimal(16384),
        upgcsl07: new Decimal(32768),
        upgcsl08: new Decimal(65536),
        ...
        hdrn30e: new Decimal(0),
        hdrn40e: new Decimal(0),
        hdrn60e: new Decimal(0),
        hdrn4wps: new Decimal(0),
        hdrn6wps: new Decimal(0),
        hdrn8wps: new Decimal(0),
        hdrn12wps: new Decimal(0),
        hdrn20wps: new Decimal(0),
        hdrn30wps: new Decimal(0),
        hdrn40wps: new Decimal(0),
        hdrn60wps: new Decimal(0),
        wxzfe: new Decimal(1),
        hydfe: new Decimal(1),
        xkwze: new Decimal(1),
        ...
        Rlitpd: new Decimal(0),
        Blitpd: new Decimal(0),
        glucps: new Decimal(0),
        frucps: new Decimal(0),
        starps: new Decimal(0),
        sucrps: new Decimal(0),
        gluce1: new Decimal(1),
        fruce1: new Decimal(1),
        stare1: new Decimal(1),
        sucre1: new Decimal(1),
        FEexp: new Decimal(1),
        ...
        slwp01: new Decimal(0),
        slwp02: new Decimal(0),
        slwp03: new Decimal(0),
        slwp04: new Decimal(0),
        slwp05: new Decimal(0),
        slwp06: new Decimal(0),
...
function getOrCreateState() {
    if (!playerInstance) {
        playerInstance = reactive(createPlayer())
        player = playerInstance
    }
    if (!vInstance) {
        vInstance = reactive(createV())
        v = vInstance
    }
    return { player: playerInstance, v: vInstance }
}
...
export function useGameState() {
    ...
    const { player, v } = getOrCreateState()
    
    const zerotosixtyfour = computed(() => {
        return Array.from({ length: 64 }, (_, i) => ({ id: i + 1 }))
    })
    ...
    function wscUnlocked() {
        if (player.hasUnlockedPL7) return 56
    ...
    const wsColor = `333322332232233223323222 
    ...
    function getWsColor(id) {
        const color = wsColor.slice(id * 3 - 3, id * 3)
        return "#" + color.replaceAll("3", "ff").replaceAll("2", "aa").replaceAll("1", "55").replaceAll("0", "00")
    }
    
    function wscale(tier) {
        const name = tiername[tier]
        const wscb = player["wscb" + name] || new Decimal(0)
        const wsccsl = v["wsccsl" + name] || new Decimal(1)
        if (wscb.gte(new Decimal(1125899906842624).div(wsccsl))) return "九阶折算|"
        else if (wscb.gte(new Decimal(35184372088832).div(wsccsl))) return "八阶折算|"
        else if (wscb.gte(new Decimal(1099511627776).div(wsccsl))) return "七阶折算|"
        else if (wscb.gte(new Decimal(34359738368).div(wsccsl))) return "六阶折算|"
        else if (wscb.gte(new Decimal(1073741824).div(wsccsl))) return "五阶折算|"
        else if (wscb.gte(new Decimal(33554432).div(wsccsl))) return "四阶折算|"
        else if (wscb.gte(new Decimal(1048576).div(wsccsl))) return "三阶折算|"
        else if (wscb.gte(new Decimal(32768).div(wsccsl))) return "二阶折算|"
        else if (wscb.gte(new Decimal(1024).div(wsccsl))) return "一阶折算|"
        else return ""
    }
    
    const chineseordinal = "零壹贰叁肆伍陆柒捌玖"
    
    function toChineseOrdinal(id) {
        if (id < 0) {
            return "负" + toChineseOrdinal(-id)
        }
        const iddigit = Math.floor(Math.log10(id))
        if (iddigit >= 1) {
    ...
    const layername = ["扩散", "扪敤", "扫敥", "扬敦", "扭敧", "扮敨", "扯敩"]
    ...
    function displayWSdata(id) {
        ...
        const name = tiername[id]
        const wsca = player["wsca" + name] || new Decimal(0)
        const wscm = v["wscm" + name] || new Decimal(1)
        const wscp = v["wscp" + name] || new Decimal(1)
        const wsch = v["wsch" + name] || new Decimal(1)
        return "+" + notation(wsca).padEnd(15, '\u00a0')
            + "×" + notation(wscm).padEnd(15, '\u00a0')
            + "^" + notation(wscp).padEnd(15, '\u00a0')
            + "\u00b6" + notation(wsch).padEnd(15, '\u00a0')
    }
...
//#region 存档
function save() {
    localStorage.windSpiritCreation = btoa(JSON.stringify(playerInstance))
    shownoti("#autosave")
}
...
function load() {
    if (localStorage.windSpiritCreation) {
        try {
            const savedData = JSON.parse(atob(localStorage.windSpiritCreation))
            transformToDecimal(savedData)
            Object.assign(playerInstance, savedData)
        } catch (e) {
...
function exportSave() {
    save()
    shownoti("#export")
    return btoa(JSON.stringify(playerInstance))
}

function exportSaveFile() {
    let str = btoa(JSON.stringify(playerInstance))
...
function importSave(saveString) {
    try {
        const savedData = JSON.parse(atob(saveString))
        transformToDecimal(savedData)
        Object.assign(playerInstance, savedData)
        save()
        shownoti("#import")
...
function importSaveFile() {
    ...
    a.onchange = () => {
        ...
        fr.onload = () => {
            let save2 = fr.result
            try {
                const savedData = JSON.parse(atob(save2))
                transformToDecimal(savedData)
                Object.assign(playerInstance, savedData)
                save()
                shownoti("#importfile")
    ...
    if (tier <= 8) {
        ...
        if (player.energy.gte(v["wscc" + name])) {
            ...
            if (player.innormcha == 5 & tier>1) {
                    ...
                    player["wsca" + name] = new Decimal(0);
...
function buyMaxWsc(tier) {
    ...
    if (tier <= 8) {
        ...
        if (player.innormcha == 2) wsccmax = wsccmax.div(1.5);
        if (wsccmax.gt(player["wscb" + name])) {
            ...
            if(player.normchacom.includes(false)) player.energy = player.energy.sub(v["wscc" + name]);
            player["wscb" + name] = player["wscb" + name].add(1);
            if (player.innormcha == 3) player.normchapow = new Decimal(0);
            if (player.innormcha == 5 & tier>1) {
                    let name = tiername[tier-1];
        ...
        if (wsccmax.gt(player["wscb" + name])) {
            player["wsca" + name] = player["wsca" + name].add(wsccmax.sub(player["wscb" + name]));
            player["wscb" + name] = wsccmax.sub(1);
            if(player.PL2upg01.lt(8)) player.PL1pts = player.PL1pts.sub(v["wscc" + name]);
            player["wscb" + name] = player["wscb" + name].add(1);
        }
        else return
    }
...
function getWscMult() {
    if(player.magstorm==true){
        for (let tier = 1; tier <= 48; tier++) {
            let name = tiername[tier];
            v["wscm" + name] = player["wscb" + name].pwb(2).max(1);
        }
        return;
    }

    let mult01to08 = Decimal.dOne;
    let mult09to16 = Decimal.dOne;
    let mult17to24 = Decimal.dOne;
    let mult25to32 = Decimal.dOne;
    let mult33to40 = Decimal.dOne;
    let mult41to48 = Decimal.dOne;

    let wmpb01to08 = Decimal.dOne;
    let wmpb09to16 = Decimal.dOne;
    let wmpb17to24 = Decimal.dOne;
    let wmpb25to32 = Decimal.dOne;
    let wmpb33to40 = Decimal.dOne;
    ...
    if (player.orbupg[0] == true) mult01to08 = mult01to08.mul(player.PL1energy.add(1).log(2).pow(14));
    if (player.orbupg[1] == true) mult01to08 = mult01to08.mul(player.energy.add(1).log(2).pow(14.5));
    if (player.orbupg[2] == true) mult09to16 = mult09to16.mul(player.PL1energy.add(1).log(2));
    if (player.orbupg[3] == true) mult09to16 = mult09to16.mul(player.energy.add(1).log(2.5));
    ...
    if (player.tier03.gte(1)) mult17to24 = mult17to24.mul(v.upgf01.add(v.upgf02.mul(2)).add(v.upgf03.mul(4)).add(v.upgf04.mul(8)).pwb(2));
    ...
    mult33to40 = mult33to40.mul(player.anm4.pow(player.anm4u02.mul(1048576)).max(1));
    ...
    if (player.tier02.gte(750)) {
        ...
        wmpb01to08 = v.wscmpb.pow(1.1);
        wmpb09to16 = v.wscmpb.pow(1.05);
        wmpb17to24 = v.wscmpb;
        wmpb25to32 = v.wscmpb;
        wmpb33to40 = v.wscmpb;
        wmpb41to48 = v.wscmpb;
    ...
    if (player.zzltu[4] == true) {
        if (player.incyzb == true & player.shdwsl[5] == true) {
            wmpb01to08 = wmpb01to08.mul(v.PL1engmul.pow(v.bpmpb.div(1.5)));
            wmpb09to16 = wmpb09to16.mul(v.PL2engmul.pow(v.bpmpb.div(1.5)));
            wmpb17to24 = wmpb17to24.mul(v.PL3engmul.pow(v.bpmpb.div(1.5)));
            wmpb25to32 = wmpb25to32.mul(v.PL4engmul.pow(v.bpmpb.div(1.5)));
            wmpb33to40 = wmpb33to40.mul(v.PL5engmul.pow(v.bpmpb.div(1.5)));
        }
        else{
    ...
    if (player.innormcha == 10) {
        for (let tier = 1; tier <= 8; tier++) {
            let name = tiername[tier];
            v["wscm" + name] = v["wscm" + name].div(v.wscbaseValue1.pwb(2));
        }
    }
}
...
function getWscPow() {
    if(player.magstorm==true){
        for (let tier = 1; tier <= 48; tier++) {
            let name = tiername[tier];
            v["wscp" + name] = Decimal.dOne;
        }
        return;
    }

    let powe01to08 = Decimal.dOne;
    let powe09to16 = Decimal.dOne;
    let powe17to24 = Decimal.dOne;
    let powe25to32 = Decimal.dOne;
    let powe33to40 = Decimal.dOne;
    let powe41to48 = Decimal.dOne;

    let wppb01to08 = Decimal.dZero;
    let wppb09to16 = Decimal.dZero;
    let wppb17to24 = Decimal.dZero;
    let wppb25to32 = Decimal.dZero;
    let wppb33to40 = Decimal.dZero;
    let wppb41to48 = Decimal.dZero;
    ...
    if (player.tier03.gte(3)) {
        powe01to08 = powe01to08.add(player.tier03.min(player.tier03.pow(0.5).mul(4)).div(400));
        powe09to16 = powe09to16.add(player.tier03.min(player.tier03.pow(0.5).mul(4)).div(400));
    }
    if (player.tier04.gte(3)) powe17to24 = powe17to24.add(player.tier03.min(player.tier03.pow(0.5).mul(4)).div(400));
    if (player.tier03.gte(106)) powe25to32 = powe25to32.add(player.tier03.min(player.tier03.pow(0.5).mul(4)).div(400));

    if (player.std[20] == true) powe01to08 = powe01to08.add(player.alch01.div(1e6).min(0.01));
    if (player.std[22] == true) powe01to08 = powe01to08.add(player.alch02.div(1e6).min(0.01));
    if (player.std[24] == true) powe09to16 = powe09to16.add(player.alch03.div(1e6).min(0.01));
    if (player.std[26] == true) powe09to16 = powe09to16.add(player.alch04.div(1e6).min(0.01));
    if (player.std[28] == true) powe17to24 = powe17to24.add(player.alch05.div(1e6).min(0.01));
    if (player.std[30] == true) powe17to24 = powe17to24.add(player.alch06.div(1e6).min(0.01));

        powe01to08 = powe01to08.add(player.alcu03.mul(0.002));
        powe09to16 = powe09to16.add(player.alcu04.mul(0.002));
        powe17to24 = powe17to24.add(player.alcu05.mul(0.002));

        powe01to08 = powe01to08.add(player.anm2u04.min(player.anm2u04.pow(0.5).mul(1.414)).min(player.anm2u04.pow(0.25).mul(2.828)).mul(0.01));
        powe09to16 = powe09to16.add(player.anm2u04.min(player.anm2u04.pow(0.5).mul(1.414)).min(player.anm2u04.pow(0.25).mul(2.828)).mul(0.01));
        powe17to24 = powe17to24.add(player.anm2u04.min(player.anm2u04.pow(0.5).mul(1.414)).min(player.anm2u04.pow(0.25).mul(2.828)).mul(0.01));
        ...
        if (player.incyzb != true | player.shdwsl[1] != true) {
            ...
            if (player.glucam.gte(1e16)) powe33to40 = powe33to40.mul(v.tthpow.max(1));
            if (player.frucam.gte(1e16)) powe41to48 = powe41to48.mul(v.tthpow.max(1));
        }
        wppb01to08 = v.hdrn4e;
        wppb09to16 = v.hdrn6e;
        wppb17to24 = v.hdrn8e;
    ...
    if (player.innormcha == 9) {
            for (let tier = 1; tier <= 8; tier++) {
                let name = tiername[tier];
                v["wscp" + name] = new Decimal(1).sub(v.wscbaseValue1.mul(0.0016)).max(0);
            }
        }
    if (player.incha == 2 | player.incha == 9) {
        for (let tier = 1; tier <= 32; tier++) {
            let name = tiername[tier];
            v["wscp" + name] = v["wscp" + name].div(player.energy.max(2).log(2).pow(0.25).div(100).add(1)).min(1);
        }
    }
}
...
function getWscHyp() {
    if(player.magstorm==true){
        for (let tier = 1; tier <= 48; tier++) {
            let name = tiername[tier];
            v["wsch" + name] = Decimal.dOne;
        }
        return;
    }

    let hypw01to08 = Decimal.dOne;
    let hypw09to16 = Decimal.dOne;
    let hypw17to24 = Decimal.dOne;
    let hypw25to32 = Decimal.dOne;
    let hypw33to40 = Decimal.dOne;
    let hypw41to48 = Decimal.dOne;
...
function getWscMultPerBuy() {
    ...
    if (player.innormcha == 12) v.wscmpb = Decimal.dOne;
}
...
function getWscCost(tier) {
    let name = tiername[tier];
    var bcost = v["wsccor" + name].add(v["wsccsl" + name].mul(player["wscb" +name]));
    v["wscc" + name] = new Decimal(2).pow(scale(bcost));
    if (player.innormcha == 2 && tier <= 8) {
        let name = tiername[tier];
        var bcost = v["wsccor" + name].add(v["wsccsl" + name].mul(player["wscb" + name]).mul(1.5));
        v["wscc" + name] = new Decimal(2).pow(scale(bcost));
    }
}

function getWscBaseValue() {
    v.wscbaseValue1 = Decimal.dOne;
    v.wscbaseValue2 = Decimal.dOne;
    v.wscbaseValue3 = Decimal.dOne;
    v.wscbaseValue4 = Decimal.dOne;
    v.wscbaseValue5 = Decimal.dOne;
    v.wscbaseValue6 = Decimal.dOne;
    v.wscbaseValue7 = Decimal.dOne;
    v.wscbaseValue8 = Decimal.dOne;
    v.wscbaseValueExp = Decimal.dOne;
    for (let tier = 1; tier <= 8; tier++) {
        let name = tiername[tier];
...
function getTierCost(tier) {
    v["tierc0" + tier] = scaleTier(player["tier0" + tier].add(1)).mul(4);
}
...
function buyMaxUpgd(tier) {
    ...
    let name = tiername[tier];
    getUpgdCost(tier);
    let max = invscale(player.energy.max(1).log(2)).add(v["upgcor" + name]).div(v["upgcsl" + name]).floor();
    if(tier>=5 & tier<=8) max = invscale(player.energy.max(2).log(2).log(2)).add(v["upgcor" + name]).div(v["upgcsl" + name]).floor();
...
function getUpgdMult() {
    ...
    if (player.hasUnlockedPL5) {
        upgepw = upgepw.mul(v.xbme05);
        upgepw2 = upgepw2.mul(v.xbme06);
    }
...
function getUpgdCost(tier) {
    let bcost = v["upgcor0"+tier].add(v["upgcsl0"+tier].mul(player["upgd0"+tier]));
    v["upgc0"+tier] = scale(bcost).pwb(2);
}
...
function PL1FR() {
        ...
        player.energy = new Decimal(2);
        for(let i=1;i<=8;i++){
            player["wscb"+tiername[i]]=new Decimal(0);
            player["wsca"+tiername[i]]=new Decimal(0);
            v["wscm"+tiername[i]] = new Decimal(1);
        }
        for(let i=9;i<=16;i++) player["wsca"+tiername[i]] = player["wscb"+tiername[i]];
        player.tier01 = new Decimal(0);
        player.tier02 = new Decimal(0);
...
function getPL1u() {
    ...
    if (player.PL1ptsttl.gte(65536)) v.PL1ue6 = player.PL1ptsttl.max(2).log(2).pow(4).max(1);
    else v.PL1ue6 = player.PL1ptsttl.max(1);
    if (player.PL1tms.gte(16)) v.PL1ue7 = player.PL1tms.root(4).mul(32768).max(1);
...
function getBab() {
    ...
    v.bethps = player.PL1energy.root(160).mul(player.chacom02.pwb(2));
...
function getChaGoal() {
    ...
    v.chagoa04 = scaleChal(player.chacom04.add(1)).pow(2).pwb("1e2500").mul("1e27500");
    v.chagoa05 = scaleChal(player.chacom05.add(1)).pow(2).pwb("e4e5");
    v.chagoa06 = scaleChal(player.chacom06.add(1)).pow(2).pwb("e6e6");
    v.chagoa07 = scaleChal(player.chacom07.add(1)).pow(2).pwb("e1.2e7");
    v.chagoa08 = scaleChal(player.chacom08.add(1)).pow(2).pwb("e2.5e7");
    v.chagoa09 = player.chacom09.add(1).pwb("1e30").mul(1e120);
    ...
    v.chagoa17 = scaleChal(player.chacom17.add(1)).pow(2).pwb("10");
    v.chagoa18 = scaleChal(player.chacom18.add(1)).pow(2).pwb("e1e21");
    v.chagoa19 = scaleChal(player.chacom19.add(1)).pow(2).pwb("e1e24");
    v.chagoa20 = scaleChal(player.chacom20.add(1)).pow(2).pwb("e1e27");
    ...
    v.charew17 = player.chacom17.min(player.chacom17.pow(0.5).mul(2)).mul(0.25);
    v.charew18 = player.chacom18.pwb(1.189);
    v.charew19 = player.chacom19.pwb(4);
    v.charew20 = player.chacom20.pwb(32);

    v.PL1chacap = new Decimal(4);
    if (player.tier04.gte(1)) v.PL1chacap = v.PL1chacap.add(player.tier04.mul(2).min(12));
    v.PL1chacap = v.PL1chacap.add(player.ptnu03.mul(5));
    if (player.synu12.gte(1)) v.PL1chacap = v.PL1chacap.add(28);
    if(player.quau[17]) v.PL1chacap = v.PL4chacap.add(36);
    v.PL2chacap = new Decimal(4).add(player.ptnu03.mul(3));
    if (player.synu12.gte(1)) v.PL2chacap = v.PL2chacap.add(20);
    if(player.quau[17]) v.PL2chacap = v.PL4chacap.add(28);
    v.PL3chacap = new Decimal(4);
    if (player.synu12.gte(1)) v.PL3chacap = v.PL3chacap.add(12);
    if(player.quau[17]) v.PL3chacap = v.PL4chacap.add(20);
    v.PL4chacap = new Decimal(4);
    if(player.quau[17]) v.PL4chacap = v.PL4chacap.add(12);
    v.PL5chacap = new Decimal(4);

    v.baseprd = Decimal.pow(2,player.normchacom.filter(Boolean).length).mul(0.25);
}
...
function comCha() {
    ...
    else if (player.incha > 16 & player.incha <= 20) {
        let name = tiername[player.incha];
        if (player.energy.gte(v["chagoa" + name]) & player["chacom" + name].lt(v.PL5chacap)) player["chacom" + name] = player["chacom" + name].add(1);
    }
}
...
function PL2reset(){
 ...
 function PL2FR() {
    ...
    for(let i=16;i<=24;i++) player["wsca" + tiername[i]] = player["wscb" + tiername[i]];
    player.tier01 = new Decimal(0);
    player.tier02 = new Decimal(0);
    player.upgd01= new Decimal(0);
    player.upgd02= new Decimal(0);
    ...
    if (player.PL2tms.lte(2)) for(let i=1;i<=12;i++) player["PL1bab" + tiername[i]] = new Decimal(0);
...
function getelmt() {
    player.elmttl = invscale(player.PL2ptsttl.max(1).log(2).pow(2.5)).pow(0.4).floor();
    player.elmt = player.elmttl.sub(player.elmt01).sub(player.elmt02).sub(player.elmt03).sub(player.elmt04).max(0);

    v.elmmpd = N(4).mul(player.chacom07.div(16).pwb(2)).mul(new Decimal(1.044273782427414).pow(player.resa02.pow(0.5)));
    let elmmul = N(1);
    if (player.tier03.gte(7)) elmmul = elmmul.mul(player.elmttl.max(1).pow(3));
    if (player.tier03.gte(16)) elmmul = elmmul.mul(player.energy.max(2).log(2));
    v.elmtps01 = v.elmmpd.pow(player.elmt01).mul(player.elmt01).mul(elmmul);
    v.elmtps02 = v.elmmpd.pow(player.elmt02).mul(player.elmt02).mul(elmmul);
    v.elmtps03 = v.elmmpd.pow(player.elmt03).mul(player.elmt03).mul(elmmul);
    v.elmtps04 = v.elmmpd.pow(player.elmt04).mul(player.elmt04).mul(elmmul);
    if (player.alcu[2] == true) {
        v.elmtps01 = v.elmmpd.pow(player.elmttl).mul(player.elmttl).mul(elmmul);
        v.elmtps02 = v.elmmpd.pow(player.elmttl).mul(player.elmttl).mul(elmmul);
        v.elmtps03 = v.elmmpd.pow(player.elmttl).mul(player.elmttl).mul(elmmul);
        v.elmtps04 = v.elmmpd.pow(player.elmttl).mul(player.elmttl).mul(elmmul);
    }

    let upgfmul = N(1);
    if (player.resa04.gte(1)) upgfmul = upgfmul.mul(v.rese04.mul(0.01).add(1));
    if (player.tthpu[3] == true) upgfmul = upgfmul.mul(v.xyze1.mul(0.01).add(1));
    if (player.xkwz.gte(0)) upgfmul = upgfmul.mul(v.xkwze.mul(0.01).add(1));
    v.upgf01 = player.elmten01.max(1).log(2).mul(upgfmul).floor();
    v.upgf02 = player.elmten02.max(1).log(4).mul(upgfmul).floor();
    v.upgf03 = player.elmten03.max(1).log(16).mul(upgfmul).floor();
    v.upgf04 = player.elmten04.max(1).log(256).mul(upgfmul).floor();
...
function buystd(tier) {
    ...
    if (tier == 19 & player.thrm.gte(54) & player.std[18] == true) { player.std[19] = true; player.thrm = player.thrm.sub(54) }
...
function ulAnm2() {
    if (player.PL2pts.gte("1e10000") & player.hasunlockedanm2 == false) player.hasunlockedanm2 = true;
}
...
function PL3reset() {
 ...
 function PL3FR() {
    ...
    for(let i=25;i<=32;i++) player["wsca" + tiername[i]] = player["wscb" + tiername[i]];
    player.tier01 = new Decimal(0);
    player.tier02 = new Decimal(0);
    player.tier03 = new Decimal(0);
    player.tier04 = new Decimal(0);
...
function gettth() {
    ...
    if (player.hasUnlockedPL5) v.tthseedpd = v.tthseedpd.pow(v.xbme07);
...
function getconv() {
    ...
    v.convp01 = player.energy.max(2).log(2).max(2).log(2).div(1200).mul(v.fraue07);
    v.convp02 = player.PL1pts.max(2).log(2).max(2).log(2).div(1000).mul(v.fraue07);
    v.convp03 = player.PL2pts.max(2).log(2).max(2).log(2).div(800).mul(v.fraue07);
    v.convp04 = player.PL3pts.max(2).log(2).max(2).log(2).div(400).mul(v.fraue07);
    v.convp05 = player.energy.max(2).log(2).max(2).log(2).div(1200).mul(v.fraue07);
    v.convp06 = player.PL1pts.max(2).log(2).max(2).log(2).div(1000).mul(v.fraue07);
    v.convp07 = player.PL2pts.max(2).log(2).max(2).log(2).div(800).mul(v.fraue07);
    v.convp08 = player.PL3pts.max(2).log(2).max(2).log(2).div(400).mul(v.fraue07);
    v.convp09 = player.energy.max(2).log(2).max(2).log(2).div(1200).mul(v.fraue07);
    v.convp10 = player.PL1pts.max(2).log(2).max(2).log(2).div(1000).mul(v.fraue07);
    v.convp11 = player.PL2pts.max(2).log(2).max(2).log(2).div(800).mul(v.fraue07);
    v.convp12 = player.PL3pts.max(2).log(2).max(2).log(2).div(400).mul(v.fraue07);
    v.convp13 = player.energy.max(2).log(2).max(2).log(2).div(1200).mul(v.fraue07);
    v.convp14 = player.PL1pts.max(2).log(2).max(2).log(2).div(1000).mul(v.fraue07);
    v.convp15 = player.PL2pts.max(2).log(2).max(2).log(2).div(800).mul(v.fraue07);
    v.convp16 = player.PL3pts.max(2).log(2).max(2).log(2).div(400).mul(v.fraue07);
}
...
function PL4reset() {
 ...
 function PL4FR() {
    ...
    for(let i=33;i<=40;i++) player["wsca" + tiername[i]] = player["wscb" + tiername[i]];
...
function getPL4tms() {
    ...
    if (player.PL4goal[0] == true) baset = baset.mul(2);
    if (player.PL4goal[1] == true) baset = baset.mul(2);
    if(player.quau[4] == true)baset = baset.mul(player.PL3tms.root(16).max(1));
    return baset;
}
...
function getboggle() {
    ...
    if (player.staram.gte(1e16)) {
        v.hdrn30wps = player.bogglettl.min(player.bogglettl.mul(16777216).root(4)).pwb(2).mul(player.bogglettl);
        v.hdrn30e = player.hdrn30w.div(4).add(2).log(2).pow(0.4).div(1024).min(0.05);
    }
    if (player.hasUnlockedPL5) {
        v.hdrn4wps = v.hdrn4wps.pow(v.xbme08);
        v.hdrn6wps = v.hdrn6wps.pow(v.xbme08);
        v.hdrn8wps = v.hdrn8wps.pow(v.xbme08);
...
function getbox1() {
    if (player.PL4pts.gte(v.boxc1)) {
        let ttl = player.PL4pts.max(4).log(4).sub(4).min(player.PL4pts.max(2).log(2).log(1.04)).floor().add(1);
...
function getbox2() {
    if (player.PL4fra.gte(v.boxc2)) {
        let ttl = player.PL4fra.max(4).root(4).sub(4).min(player.PL4fra.max(1).log(1.1)).floor().add(1);
...
function getbatow() {
    ...
    if (v.ntma.gte(152)) {
        v.skef01 = player.PL4energy.max(2).log(2).pow(player.skptcp.mul(0.1667));
        v.skef02 = player.PL4energy.max(2).log(2).pow(player.skptcp.mul(0.1667));
        v.skef03 = player.hdrn4w.max(2).log(2).pow(player.skptcp.mul(0.6667));
        v.skef04 = player.hdrn4w.max(2).log(2).pow(player.skptcp.mul(0.6667));
    }
    else {
        v.skef01 = player.PL4energy.max(2).log(2).pow(player.sklv01.mul(0.3333));
        v.skef02 = player.PL4energy.max(2).log(2).pow(player.sklv02.mul(0.3333));
        v.skef03 = player.hdrn4w.max(2).log(2).pow(player.sklv03.mul(1.3333));
        v.skef04 = player.hdrn4w.max(2).log(2).pow(player.sklv04.mul(1.3333));
    }
    ...
    if (player.quau[10] == true) player.skptcp = v.skptpd;
}

function hpstg(stg){
    let df = stg.sub(1).div(1000).min(10).floor();
    let hps = df.pwb(2).mul(stg.add(24)).sub(df.pwb(2).mul(df).sub(df.pwb(2).sub(1)).mul(1024));
    return hps.div(16).pwb(2);
}
...
function getsyn() {
    v.synps01 = Decimal.dOne;
    v.synps02 = Decimal.dOne;
    v.synps03 = Decimal.dOne;
    v.synps04 = Decimal.dOne;
    v.synps05 = Decimal.dOne;
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        if (player.quau[7] == true) buysynu(i);
    }
...
function getntm() {
    ...
    v.ntmm = player.nrev.mul(0.05).add(1);
    ...
    for (let i = 1; i <= 6; i++) {
        ...
        if (i == 1) v["ntme" + name] = v.ntma.min(v.ntma.pow(0.5).mul(3.1622)).mul(0.01);
        else v["ntme" + name] = v.ntma.pow(0.5).min(v.ntma.pow(0.2).mul(4)).mul(0.0001);
    }
...
function ntmact() {
    ...
    let i = (Math.floor(Math.random() * (6-player.ntmu04.toNumber()))) + player.ntmu04.toNumber();
...
function PL5reset() {
 ...
 function PL5FR() {
    ...
    if (player.quau[4] == false) {
        for (let i = 0; i <= 15; i++) player.PL4goal[i] = false;
    }
    if (player.PL4goal[2] == true) player.PL2tms = new Decimal(12);
    if (player.quau[1] == true & player.PL2tms.eq(0)) player.PL2tms = new Decimal(4);
    ...
    for(let i=41;i<=48;i++) player["wsca" + tiername[i]] = player["wscb" + tiername[i]];
    ...
    if (player.quau[3] != true & player.PL2tms.lte(7)) {
        player.chacom01 = new Decimal(0);
        player.chacom02 = new Decimal(0);
        player.chacom03 = new Decimal(0);
        player.chacom04 = new Decimal(0);
    ...
    player.PL2pts = new Decimal(0);
    player.PL2ptsttl = new Decimal(0);
    if (player.PL4goal[4] == true) player.PL2tms = new Decimal(12);
    player.PL2sec = new Decimal(0);
    player.PL2tmsc = new Decimal(0);
    if (player.quau[3] != true & player.PL4goal[4] != true) {
        player.PL2upg01 = new Decimal(0);
        player.PL2upg02 = new Decimal(0);
        player.PL2upg03 = new Decimal(0);
        player.PL2upg04 = new Decimal(0);
    ...
    player.PL3ptsttl = new Decimal(0);
    player.PL3sec = new Decimal(0);
    if (player.quau[3] != true & player.PL4goal[9] != true) {
        player.chacom09 = new Decimal(0);
        player.chacom10 = new Decimal(0);
        player.chacom11 = new Decimal(0);
        player.chacom12 = new Decimal(0);
    ...
    if (player.quau[6] != true){
        player.stgmx= new Decimal(1);
        if (player.quau[12] != true) player.shdwrc= new Decimal(0);
        ...
        player.stgmx= new Decimal(2000);
        if (player.quau[12] != true) player.shdwrc= new Decimal(1024);
        player.ntmv= new Decimal(300);
        player.nrev = new Decimal(30);
        player.ntmu03= new Decimal(9);
        player.ntmu04= new Decimal(3);
    }
    ...
    if (player.quau[6] != true) player.hasunlockedanm4= false,
    player.incyzb= false,
    player.shdwsl= [false,
        false, false,
        false, false, false,
...
function buyquau(tier) {
    ...
    if (tier == 3 & player.quant.gte(1e8) & player.quau[0]) {
        player.quau[3] = true;
        for (let i = 0; i < 16; i++)player.PL4goal[i] = true;
    }
    if (tier == 4 & player.quant.gte(1e10) & player.quau[1]) {
        player.quau[4] = true;
        player.alcu[1] = true;
        player.xyzbo = N(2);
        player.zzltu[0] = true;
    }
    if (tier == 5 & player.quant.gte(1e12) & player.PL5tms.gte(2)) {
        player.quau[5] = true;
    }
    if (tier == 6 & player.quant.gte(1e13) & player.PL5tms.gte(2)) {
        player.quau[6] = true;
    }
    if (tier == 7 & player.quant.gte(1e11) & player.quau[2]) {
        player.quau[7] = true;
    }
    if (tier == 8 & player.quant.gte(1e18) & player.PL5tms.gte(10)) {
        player.quau[8] = true;
    }
    if (tier == 9 & player.quant.gte(1e20) & player.quau[3]) {
        player.quau[9] = true;
    }
    if (tier == 10 & player.quant.gte(1e22) & player.quau[4]) {
        player.quau[10] = true;
    }
    if (tier == 11 & player.quant.gte(1e24) & player.quau[5]) {
        player.quau[11] = true;
    }
    if (tier == 12 & player.quant.gte(1e26) & player.quau[6]) {
        player.quau[12] = true;
    }
    if (tier == 13 & player.quant.gte(1e28) & player.energy.gte("e5e27")) {
        player.quau[13] = true;
    }
    ...
    if (tier == 16 & player.quant.gte(1e60)) {
        player.quau[16] = true;
    }
    if (tier == 17 & player.quant.gte(1e80)) {
        player.quau[17] = true;
    }
}
...
function buyxbmu(tier) {
    ...
    let name = tiername[tier];
    if (tier == 1 | tier == 3) {
        if (player.xbmu01.gte(10)) return;
        if (player.xbmu03.gte(11)) return;
...
function getphs() {
    if (player.Rlitengp) v.Rlitpd = v.Rlitpd.add(0.05).mul(1.035264923841377);
    if (v.Rlitpd.gte(player.PL1sec.pow(0.25))) {
        player.Rlitengp = false;
        player.Rliteng = player.Rliteng.add(v.Rlitpd);
        v.Rlitpd = N(0);
    }
    if (player.Blitengp) v.Blitpd = v.Blitpd.add(0.05).mul(1.035264923841377);
    if (v.Blitpd.gte(player.PL5sec)) {
        player.Blitengp = false;
        player.Bliteng = player.Bliteng.add(v.Blitpd);
        v.Blitpd = N(0);
    }
    v.glucps = player.Rliteng.mul(player.Bliteng).hyp(0.8);
    v.frucps = player.glucam.hyp(0.8);
    v.starps = player.frucam.hyp(0.8);
    v.sucrps = player.staram.hyp(0.8);
    v.gluce1 = player.glucam.pow(0.5).max(2);
    v.fruce1 = player.frucam.pow(0.25).max(2);
    v.stare1 = player.staram.pow(0.125).max(2);
    v.sucre1 = player.sucram.pow(0.0625).max(2);
}

function colRlit() {
    if (!player.Rlitengp) player.Rlitengp = true;
}

function colBlit() {
    if (!player.Blitengp) player.Blitengp = true;
}
...
function emeadd1() {
    player.EMenergy = player.EMenergy.add(1);
}

function buyslw(tier) {
    let name = tiername[tier];
    getslw();
...
function swMagStorm(){
    ...
    for(let i=1;i<=48;i++){
        ...
        player["wscb"+name]=Decimal.dZero;
    }
    player.energy=N(2);
    if(player.magstorm) player.magstorm=false;
...
function updateBaseProduction() {
    if (!player.hasUnlockedBases) {
        player.hasUnlockedBases = true;
    }
...
function canBuyBase(baseName) {
    if (!player[baseName + '_unlocked']?.[0]) return false;
    let cost = getBaseCost(baseName);
    return player.PL5pts.gte(cost);
}

function buyBase(baseName) {
    if (!canBuyBase(baseName)) return;
    let cost = getBaseCost(baseName);
    player.PL5pts = player.PL5pts.sub(cost);
    player[baseName] = player[baseName].add(1);
    updateBaseProduction();
}

function getBaseUpgrade1Cost() {
    let totalLevel = player.baseA_upg1.add(player.baseG_upg1).add(player.baseC_upg1).add(player.baseT_upg1).add(player.baseU_upg1);
    return new Decimal(10).pow(totalLevel);
}

function getBaseUpgrade2Cost() {
    let totalLevel = player.baseA_upg2.add(player.baseG_upg2).add(player.baseC_upg2).add(player.baseT_upg2).add(player.baseU_upg2);
    return new Decimal(100).pow(totalLevel);
}

function getBaseUpgrade1Effect() {
    let totalLevel = player.baseA_upg1.add(player.baseG_upg1).add(player.baseC_upg1).add(player.baseT_upg1).add(player.baseU_upg1);
    return new Decimal(2).pow(totalLevel);
}

function getBaseUpgrade2Effect() {
    let totalLevel = player.baseA_upg2.add(player.baseG_upg2).add(player.baseC_upg2).add(player.baseT_upg2).add(player.baseU_upg2);
    return new Decimal(1.1).pow(totalLevel);
}

function buyBaseUpgrade1All() {
    let bases = ['baseA', 'baseG', 'baseC', 'baseT', 'baseU'];
    for (let base of bases) {
        buyBaseUpgrade1(base, base + '_upg1');
    }
}

function buyBaseUpgrade2All() {
    let bases = ['baseA', 'baseG', 'baseC', 'baseT', 'baseU'];
    for (let base of bases) {
        buyBaseUpgrade2(base, base + '_upg2');
    }
}
...
function PL6reset() {
 ...
 function PL6FR() {
        ...
        for(let i=49;i<=56;i++) player["wsca" + tiername[i]] = player["wscb" + tiername[i]];
...
function produce(spd = 1) {
    let speed = new Decimal(spd).mul(v.gamespd1).mul(v.glbspd).mul(diff);
    let speed2 = new Decimal(spd).mul(v.gamespd2).mul(v.glbspd).mul(diff);

    player.totalSeconds = player.totalSeconds.plus(v.glbspd*diff);
    player.PL1secrl = player.PL1secrl.plus(v.glbspd*diff);
    player.PL2secrl = player.PL2secrl.plus(v.glbspd*diff);
    player.PL3secrl = player.PL3secrl.plus(v.glbspd*diff);
    player.PL4secrl = player.PL4secrl.plus(v.glbspd*diff);
    player.PL5secrl = player.PL5secrl.plus(v.glbspd*diff);
    ...
    if (player.energy.log(2).lte(getsoftcap().log(2))) v.energyps = player.wsca01.mul(v.wscm01).pow(v.wscp01).hyp(v.wsch01).pow(v.FEexp).mul(speed).mul(v.baseprd);
    ...
    if (player.hasunlockedanmorb == true) {
        ...
        let anm1b = N(2);
        if (player.glucam.gte(1e6)) anm1b = v.gluce1;
        v.anmparps = player.wscb08.pwb(1.258925412).max(1).div(1e6).mul(player.parupg01.pwb(anm1b)).mul(player.chacom04.pwb(2));
    ...
    if (player.hasunlockedanm3 == true) {
        let anm3b = N(2);
        if (player.staram.gte(1e6)) anm3b = v.stare1;
        v.anm3ps = v.blcnt.sub(0.5).max(0).mul(0.001)
        v.anm3ps = v.anm3ps.mul(player.anm3u01.pwb(anm3b)).mul(player.chacom12.pwb(8));
    ...
    if (player.hasunlockedanm4 == true & player.incyzb == false) {
        let anm4b = N(2);
        if (player.sucram.gte(1e6)) anm4b = v.sucre1;
    ...
    if (player.hdrn4.gte(1) | player.synu13.gte(1)) player.hdrn4w = player.hdrn4w.add(v.hdrn4wps.mul(speed2));
    else player.hdrn4w = player.hdrn4w.div(1.035265);
    if (player.hdrn6.gte(1) | player.synu13.gte(1)) player.hdrn6w = player.hdrn6w.add(v.hdrn6wps.mul(speed2));
    else player.hdrn6w = player.hdrn6w.div(1.035265);
    if (player.hdrn8.gte(1) | player.synu13.gte(1)) player.hdrn8w = player.hdrn8w.add(v.hdrn8wps.mul(speed2));
    else player.hdrn8w = player.hdrn8w.div(1.035265);
    if (player.hdrn12.gte(1) | player.synu13.gte(1)) player.hdrn12w = player.hdrn12w.add(v.hdrn12wps.mul(speed2));
    ...
    if (player.zzltu[4] == true) {
        player.coin = player.coin.add(v.cps.mul(speed2));
    ...
    if (player.quau[8]) {
        player.glucam = player.glucam.add(v.glucps.mul(speed2));
        player.frucam = player.frucam.add(v.frucps.mul(speed2));
        player.staram = player.staram.add(v.starps.mul(speed2));
        player.sucram = player.sucram.add(v.sucrps.mul(speed2));
    }

    if (player.sucram.gte(1e16)) {
        player.EMenergy = player.EMenergy.mul(player.slwa01.pow(v.slwp01).root(20));
        for (let i = 2; i <= 8; i++) {
            let name = tiername[i];
...
function getspd() {
    ...
    if(player.magstorm==true)v.gamespd1 =Decimal.dOne;
...
function getsoftcap() {
    ...
    sc = sc.pow(v.fraue03);
    if (player.incyzb == true & player.shdwsl[4] == true) sc = sc.root(65536);
    return sc
}
...
function time(diff) {
    ...
    playerInstance.nowtime = Date.now();
    let diff2 = new Decimal(diff);
    playerInstance.milliseconds = playerInstance.milliseconds.plus(diff2.times(1000));
    if (playerInstance.milliseconds.gte(1000)) {
        playerInstance.seconds = playerInstance.seconds.plus(1);
        playerInstance.milliseconds = playerInstance.milliseconds.minus(1000);
    }
    if (playerInstance.seconds == 60) {
        playerInstance.seconds = new Decimal(0);
        playerInstance.minuts = playerInstance.minuts.plus(1);
    }
    if (playerInstance.minuts == 60) {
        playerInstance.minuts = new Decimal(0);
        playerInstance.hours = playerInstance.hours.plus(1);
    }
    if (playerInstance.hours == 24) {
        playerInstance.hours = new Decimal(0);
        playerInstance.days = playerInstance.days.plus(1);
    }
}
...
function epsdisp() {
    if (player.energy.div(v.preps.max(1)).root(diff).lte(2)) return (notation(player.energy.sub(v.preps).div(diff)));
    else if (player.energy.log(v.preps.max(2)).root(diff).lte(2)) return (notation(player.energy.div(v.preps).log(2).div(diff)) + " OoM");
    else return (notation(player.energy.log(v.preps).log(2).div(diff)) + " OoM²");
}

function fszqw() {
    if (v.caidan != true) alert("资源不足，无法升级！");
    else {
...
function comAch() {
    ...
    if (player.wsca01.mul(v.wscm01).div(100).lte(player.energy) & player.energy.gte(1e20)) getAch(13);
    ...
    if (!player.normchacom.includes(false)) getAch(19);
...
function mainLoop() {
    ...
    getWscHyp();
    getWscMultPerBuy();
    for (let i = 1; i <= 48; i++) getWscCost(i);
    getWscBaseValue();
    for (let i = 1; i <= 6; i++) getTierCost(i);
    getUpgdMult();
    for (let i = 1; i <= 6; i++) getUpgdCost(i);

    getspd();
    autoBuy();
    produce();
    if(player.gmrn) time(diff);
    comAch();
    comNormCha();
    comCha();
    useoflntm();
...
function autoBuy() {
    ...
    if (player.PL2tms.gte(10)) autoBuyTier03();
    if (player.alcu[0] == true) autoBuyTier04();
    if(player.quau[9] == true){
        autoBuyTier05();
        autoBuyTier06();
    }
    ...
    if (player.PL2tms.gte(8)) autoBuyUpgd04();
    if(player.quau[10] == true){
        autoBuyUpgd05();
        autoBuyUpgd06();
    }
...
function test11() {
    for (let i = 0; i < 16; i++) player.PL1upg[i] = true;
}

function test12() {
    for (let i = 1; i <= 12; i++) {
        let name = tiername[i];
        player["PL1bab" + name] = N(5);
    }
}
...
function test22() {
    for (let i = 0; i < 40; i++) player.std[i] = true;
    for (let i = 0; i < 6; i++) player.alcu[i] = true;
    for (let i = 1; i <= 6; i++) {
        let name = tiername[i];
        player["alcu" + name] = N(10);
    }
}
...
function test31() {
    for (let i = 1; i <= 16; i++) {
        let name = tiername[i];
        player["resa" + name] = N(1);
    }
}

function test32() {
    for (let i = 0; i < 5; i++) player.tthatunl[i] = true;
    for (let i = 0; i < 4; i++) player.tthpu[i] = true;
    for (let i = 0; i < 4; i++) player.anm3u[i] = true;
    player.xyzsw = N(7);
    player.xyzbo = N(3);
}
...
function startNewsTicker() {
    ...
    let l = -50 - (newsElement.innerText.length * 16)
    
    newsElement.innerHTML = texts[Math.floor(Math.random() * texts.length)]
    
    const timer = setInterval(function () {
        if (!playerInstance || !playerInstance.news) return
        p -= 1
        if (p <= l) {
            newsElement.innerHTML = texts[Math.floor(Math.random() * texts.length)]
            newsElement.style.width = (newsElement.innerText.length * 16).toString() + "px"
            l = -50 - (newsElement.innerText.length * 16)
            p = 50 + document.body.clientWidth
        }
        newsElement.style.left = p.toFixed(1) + "px"
    }, 10)
...
//#endregion
    ...
    return {
        ...
        //扬敦
        swi8n1, buyfrau, getbox1, getbox2, openbox1, openbox2, resbox, forge, resforge,  buyltwu, chgstg,
        buybatu, batpre, reskl, sklv, fcssyn, buysynu, reshsl, encyzb, shsl, shad, buyanm4u, ulAnm4, buyntmu,
        //扭敧
        chdepth, buyquau, fillqua, buyxbmu, colRlit, colBlit, emeadd1, buyslw, swMagStorm,
        buyAlchemyParent,
        buyAlchemyTool,
        startHybridization,
        selfPollinate,
        ...
        buyBaseUpgrade2All,
        checkCRISPRUnlock,
        getCRISPRDnaGeneration,
        unlockCRISPRGeneSlot,
        editCRISPRGene,
        upgradeCRISPRGene,
        getCRISPREditingCost,
        getCRISPRGeneEffect,
        upgradeCRISPREditingTech,
        //扮敨
        ...
        //杂项
        save, load, exportSave, importSave, exportSaveFile, importSaveFile, trueHardReset,
        autobuyon, autobuyoff, swfyjj, swgmrn, fszqw,
