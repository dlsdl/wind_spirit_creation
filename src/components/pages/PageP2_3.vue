<template>
  <div id="p2_3" v-if="v.currentPage=='p2_3'">
    ...
    <div id="chal5" v-if="player.quau[16]">
      <table align="center">
        <tbody>
          <tr>
            <td>
            ...
            <td>
              <button :class="player.incha==18?'chalin':player.chacom18.gte(v.PL5chacap)?'chalyes':'chalno'" style="border: 4px solid #0000ff" @click="entCha(18)">
                ...
                目标：{{notation(v.chagoa18)}}能量<br />完成次数：{{notatint(player.chacom18)}}/{{notatint(v.PL5chacap)}}<br />奖励：提升太阳风灵指数(×{{notation(v.charew18)}})
              </button>
            </td>
            <td>
              <button :class="player.incha==19?'chalin':player.chacom19.gte(v.PL5chacap)?'chalyes':'chalno'" style="border: 4px solid #0000ff" @click="entCha(19)">
                ...
                目标：{{notation(v.chagoa19)}}能量<br />完成次数：{{notatint(player.chacom19)}}/{{notatint(v.PL5chacap)}}<br />奖励：提升碱基获取数量(×{{notation(v.charew19)}})
              </button>
            </td>
            <td>
